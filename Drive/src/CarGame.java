// Yusuf Taha Öncü 230611032
// Özgül Rana Çokkayışlı 220611017
...
public class CarGame extends JFrame implements KeyListener, ActionListener {
    ...
    private boolean  gameover = false, paint = false, inMainMenu = true;
    public ImageIcon mainMenuBackground;

    public CarGame(String title) {
        super(title);
        setBounds(300, 10, 700, 1000);
        setVisible(true);
    ...
    private void playGame(Graphics g) {
        ...
        g.fillRect(90, 0, 10, 1000);
        g.fillRect(600, 0, 10, 1000);
        g.setColor(Color.black);
        g.fillRect(100, 0, 500, 1000);
    ...
    public static void main(String args[]) {
        CarGame c = new CarGame("DRIVE AND SURVIVE");
    }

    @Override
    public void keyPressed(KeyEvent e) {
        // "Enter" tuşuna basıldığında ana menüden çıkıp oyuna başla

        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            if (inMainMenu) {
                // Ana menüden çıkıp oyuna başlat
                ...
                y2pos = carypos[cypos2];
                y3pos = carypos[cypos3];
                delay = 100; // Başlangıç hızı
                repaint();  // Yeni oyun başladığında ekranı yenile
            } else if (gameover) {
                // Oyun bittiyse, 'Enter' ile yeniden başlat
                gameover = false;
                score = 0;
                speed = 90;
                xpos = 300;
        ...
        // Yön tuşlarına basıldığında arabayı hareket ettir
